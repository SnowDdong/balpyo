...
SOD_df
ggplot(data = SOD_df, aes(x = SOD_date,y = SOD_confirmed, fill = SOD_date)) + geom_bar(stat='identity') + geom_text(aes(label=SOD_confirmed),position = position_dodge(width=1),vjust=-0.5) +scale_fill_manual(values=colors)+ geom_hline(yintercept=0)
...
policy_names
policy_names <- paste(policy_names,"%")
policy_names
pie(confirm_change_abs,labels = policy_names, main = "5가지 정책 시행 후 변화율", init.angle = 90,col = piecolor)
...
df_change_df
library(ggplot2)
ggplot(data = df_change_df,aes(x = policy_name,y = confirm_change, fill = policy_name)) + geom_bar(stat='identity') +scale_fill_manual(values=allcolor)+ theme(text = element_text(size=15),axis.text.x = element_text(angle=90, hjust=1))+labs(x="정책이름", y="변화율") + geom_hline(yintercept=0)+ 
#그래프 순서 지정
scale_x_discrete(limits = c("사회적 거리두기",                                                                                        "마스크 공공5부제","해외입국자 2주 자가격리",
                            "특별 출입국 절차",
                            "학교 및 어린이집 개학(강) 연기"))
